...
import { Shield, Sparkles, Download, HelpCircle, Activity, Award, TrendingUp, AlertTriangle, FileText } from 'lucide-react';
...
export default function RiskMonteCarloTab({ assetSymbol, selectedPeriod }) {
  ...
  const mcChartData = {
    ...
    datasets: [
      ...
      {
        label: '5th Percentile (Worst-Case Drawdown)',
  ...
  const downloadReportText = () => {
    ...
    const textContent = `========================================================================
QUANTUM INSTITUTIONAL QUANTITATIVE AUDIT & RISK REPORT
Generated by QUANTUM Financial Intelligence Platform
========================================================================
Asset:                 ${assetSymbol}
Evaluation Period:     ${selectedPeriod}
Executive Grade:       ${reportData.executive_grade}
Overall Score:         ${reportData.score} / 100
Report Title:          ${reportData.report_title}
...
------------------------------------------------------------------------
RISK ANALYTICS & CAPITAL AT RISK (VaR / CVaR)
------------------------------------------------------------------------
Portfolio Value:       $100,000
Parametric VaR (95%):  -$${riskData?.risk_levels?.['95%']?.parametric_var_usd?.toLocaleString() || 'N/A'} (${riskData?.risk_levels?.['95%']?.parametric_var_pct || 'N/A'}%)
Historical VaR (95%):  -$${riskData?.risk_levels?.['95%']?.historical_var_usd?.toLocaleString() || 'N/A'} (${riskData?.risk_levels?.['95%']?.historical_var_pct || 'N/A'}%)
Expected Shortfall:    -$${riskData?.risk_levels?.['95%']?.cvar_expected_shortfall_usd?.toLocaleString() || 'N/A'} (${riskData?.risk_levels?.['95%']?.cvar_expected_shortfall_pct || 'N/A'}%)

------------------------------------------------------------------------
MONTE CARLO PROJECTIONS (500 Stochastic Paths, ${simHorizon} Days)
------------------------------------------------------------------------
Median Projected Equity: $${mcData?.median_projected_equity?.toLocaleString() || 'N/A'}
Probability of Profit:   ${mcData?.prob_positive_return_pct || 'N/A'}%
Max Projected Equity:    $${mcData?.max_projected_equity?.toLocaleString() || 'N/A'}
Min Projected Equity:    $${mcData?.min_projected_equity?.toLocaleString() || 'N/A'}

------------------------------------------------------------------------
KEY QUANTITATIVE FINDINGS
------------------------------------------------------------------------
${(reportData.key_findings || []).map((f, i) => `${i + 1}. ${f}`).join('\n')}
...
========================================================================
DISCLAIMER: For quantitative research and backtesting audit purposes only.
`;

    const dataStr = "data:text/plain;charset=utf-8," + encodeURIComponent(textContent);
    const dlAnchor = document.createElement('a');
    dlAnchor.setAttribute("href", dataStr);
    dlAnchor.setAttribute("download", `Quant_Research_${assetSymbol}_Brief.txt`);
    dlAnchor.click();
  };
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <button
                ...
                title="Download human-readable executive research summary"
              >
              ...
              <button
                ...
                style={{ padding: '0.4rem 0.8rem', fontSize: '0.75rem' }}
                title="Download programmatic raw JSON dataset for quant analysis"
              >
